...
class ChatLogController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    var puser: User? {
        didSet {
            navigationItem.title = puser?.name
            
            observeMessages()
        }
    }
    
    var pmessages = [PMessage]()
    
    func observeMessages() {
        guard let uid = Auth.auth().currentUser?.uid, let toId = puser?.userID else {
            return
        }
        ...
        userMessagesRef.observe(.childAdded, with: { (snapshot) in
            ...
            messagesRef.observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                self.pmessages.append(PMessage(dictionary: dictionary))
...
let info = convertFromUIImagePickerControllerInfoKeyDictionary(info)
    ...
    fileprivate func handleVideoSelectedForUrl(_ url: URL) {
        ...
        uploadTask.observe(.success) { (snapshot) in
            self.navigationItem.title = self.puser?.name
        }
    }
...
//            collectionView?.scrollToItem(at: indexPath, at: .top, animated: true)
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let message = pmessages[indexPath.item]
        
        cell.pmessage = message
    ...
    fileprivate func setupCell(_ cell: ChatMessageCell, message: PMessage) {
        if let profileImageUrl = self.puser?.profileImageURL {
